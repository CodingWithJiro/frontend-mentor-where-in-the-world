const Filter = () => {
  return (
    <div className="relative max-w-50 rounded-sm font-(family-name:--FF) font-light shadow-(--SHADOW-CARD-PRIMARY) motion-safe:transition-shadow motion-safe:duration-150 motion-safe:ease-in-out">
      ...
      <ul
        role="listbox"
        className="absolute top-[calc(4px+100%)] left-0 z-10 w-full rounded-sm bg-(--COLOR-BG-CARD-PRIMARY) pt-3.25 pb-3 text-[0.75rem] shadow-(--SHADOW-CARD-PRIMARY) select-none motion-safe:transition-[color,background-color,box-shadow] motion-safe:duration-150 motion-safe:ease-in-out [&_button]:w-full [&_button]:cursor-pointer [&_button]:rounded-sm [&_button]:px-6 [&_button]:py-0.75 [&_button]:text-left [&_button]:hover:bg-(--COLOR-BG-CARD-PRIMARY-HOVER) [&_button]:focus-visible:outline-1 [&_button]:focus-visible:outline-(--COLOR-OUTLINE-PRIMARY)"
      >
        <li role="option">
